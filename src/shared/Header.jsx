...
const Header = () => {
  ...
  return (
    <>
      ...
      <div
        className={`hidden w-full px-5 py-5 md:flex items-center gap-3 fixed left-0 right-0 ${
          scrolled
            ? "header-shadow z-40 bg-[#32745c] top-0 transition-bg duration-300 py-5 shadow-md"
            : ""
        }`}
      >
          ...
          <div className="flex items-center gap-5">
            {myMenu.map((item) => (
              <NavLink
                ...
                className={({ isActive }) =>
                  isActive
                    ? "text-[#b29361] text-[16px] font-semibold transition duration-300"
                    : "text-white text-[16px] font-semibold hover:text-[#b29361] transition duration-300"
                }
              >
                {item.label}
              </NavLink>
